def _simple_java_library(ctx):
    ...
    ctx.actions.run_shell(
        command = "javac -cp {} {} -d {}".format(":".join(["."] + [dep.path for dep in dep_jars.to_list()]), ctx.file.src.path, out_classdir.path),
    ...
    ctx.actions.run_shell(
        command = "jar cf $(pwd)/{} -C {} .".format(out_jar.path, out_classdir.path),
        inputs = [out_classdir],
        outputs = [out_jar],
        mnemonic = "JarClasses",
    )
